...
class Router:
    ...
    def _get_list_items(self, channels, bitrate=None, default_channel_id=None):
        ...
        def _get_streams(channel, format):
            """Get a stream from a channel element. Default to highest bitrate"""
            # attribute expression not supported in python 2.6, so this won't work
            # xpath = "streams/stream[@format='{0}']".format(format)
            # if bitrate:
            #     xpath += "[bitrate='{:d}']".format(bitrate)
            for stream in channel.findall("streams/stream"):
                if not format or stream.attrib.get('format') != format:
                    continue
                if bitrate and int(stream.attrib.get('bitrate')) != bitrate:
                    continue
        ...
        for c in channels:
            ...
            # if not streams:
            streams = _get_streams(c, format='mp3')

            # choose highest rate stream by default
            try:
                stream = max(streams, key=lambda stream: int(stream.findtext('bitrate')))
            except ValueError:
                # requires >= 1 streams
                continue
            else:
